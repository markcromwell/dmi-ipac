...
def build_geometry(mg: ModelGeometry,
    ...
    """Build bundle geometry, preferring EXACT values from the spreadsheet.

    bundle_type: 'Fixed' or 'Removable'
    dp_psig:     design pressure (200 or 300) for tubesheet thickness

    If exact geometry has been extracted for this (bundle, pressure, model)
    combination (see wcac/exact_geometry.py, generated by
    tests/spreadsheet/extract_geometry.py), those values are used directly so
    the library matches the spreadsheet exactly. Otherwise it falls back to
    computing the geometry from first principles (_compute_geometry).
    """
    dp = 300 if round(dp_psig/100)*100 >= 300 else 200
    try:
        from .exact_geometry import EXACT_GEOMETRY
        exact = EXACT_GEOMETRY.get((bundle_type, dp), {}).get(mg.code)
    except ImportError:
        exact = None

    if exact is not None:
        return _from_exact(mg, exact)
    return _compute_geometry(mg, bundle_type, dp_psig)


def _from_exact(mg: ModelGeometry, ex: dict) -> BundleGeometry:
    """Build BundleGeometry directly from extracted spreadsheet values."""
    Dto = TUBE_OD; Dti = TUBE_ID
    Xt = mg.Xt; OTL = mg.OTL
    # Row pitch
    ...
    Lt_full = mg.Lt
    Lt_eff  = ex['Lteff']
    ts_geom = (Lt_full - Lt_eff) / 2
    return BundleGeometry(
        Nt=int(round(ex['Nt'])), Xr=Xr, r_ctl=ex['rctl'],
        Lt_full=Lt_full, Lt_eff=Lt_eff, A=ex['A'], Act=ex['Act'],
        sigmat=ex['sigmat'],
        Acs=ex['Acs'], A_m_raw=ex['Acs']/1.31, Lbc=ex['Lbc'], Lbc_std=mg.Lbc_std,
        Lbe=ex['Lbe'], Nb=int(round(ex['Nb'])),
        Fw=0.0, Fc=0.0, theta_DS=0.0, theta_ctl=0.0,
        Ntcc=ex['Ntcc'], Ntcc_raw=ex['Ntcc'], Ntcw=ex['Ntcw'],
        Jc=ex['Jc'], Jl=ex['Jl'],
        A_sb=ex['Asb'], A_bt=ex['Abt'], A_bas=ex['Abas'],
        xi_SB=(mg.Dsi-mg.Dbaffle)/2, xi_BT=(mg.Dhole-Dto)/2,
        A_cw=ex['Acw'], A_cse=ex['Acse'], A_csn=ex['Acsn'], D_hw=ex['Dhw'],
...
def _compute_geometry(mg: ModelGeometry,
                      bundle_type: str = 'Fixed',
                      dp_psig: float = 300) -> BundleGeometry:
    """Compute geometry from first principles (fallback when no exact data).

    Applies verified overrides from GEOMETRY_OVERRIDES where available.
    See CALCULATION_DISCREPANCIES.md for the three Lbc values and two ts values.
    """
